...
def get_classifier_model( model_name, params=None, njobs=25 ):
    '''
    ...
    if 'Random' in model_name:
        if params is None:
            ...
            return RandomForestClassifier( random_state=42, n_jobs = njobs, n_estimators = params['n_estimators'], min_samples_leaf = params['min_samples_leaf'],
        ...
        if params is None:
            ...
            return GradientBoostingClassifier( loss = 'deviance', n_estimators = params['n_estimators'], learning_rate = params['alpha'], min_samples_leaf = 5)
    
    elif 'Logistic' in model_name:
        if params is None:
            return LogisticRegression(n_jobs=njobs, solver='saga', penalty='elasticnet', max_iter=300, random_state=42)
        else:
            return LogisticRegression(n_jobs=njobs, solver='saga', penalty='elasticnet', C = params['C'], max_iter=300, l1_ratio=params['l1_ratio'] )
    
    elif 'XGB' in model_name:
        if params is None:
            return XGBClassifier(n_jobs=50, random_state=42, objective = 'binary:logistic')
        else:
...
class MyNeuralNetworkClassifier(BaseEstimator, ClassifierMixin,
    ...
    def _build_the_network(self,):
        ...
        activation_layer_object = self._get_activation_layer( function_name =  self.activation, alpha_parameter=self.alpha_parameter )
        ...
        if self.choice['layers'] == 'three':
            ...
            dense_layer_object = self._get_dense_layer( self.choice['units3'], self.l1_weight,  self.l2_weight )
            last_layer_object  = dense_layer_object( last_layer_object )
